...
class SaveHasNewGroupMessageUseCase @Inject constructor(private val messagesLocalRepository: MessagesLocalRepository) {
    suspend operator fun invoke(hasNewMessage: Boolean) {
        messagesLocalRepository.saveHasNewGroupMessage(hasNewMessage)
    }
}